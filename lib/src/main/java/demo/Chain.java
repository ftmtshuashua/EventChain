...
import Event;
import internal.ILinkableEvent;
import listener.OnChainListener;
import utils.ListenerMap;
 ...
 * </pre>
 */
public class Chain<P, R> implements ILinkableEvent<P, R, Chain<P, R>>, OnChainListener {
    ...
    /**
     * 判断当前链是否被打断,被打断的链上的事件将无法进行事件传递。它将完成当前链并抛出一个{@link excption.EventInterruptException}异常
     *
     * @return 是否已打断
     */
    ...
    public void onEventStart(Event event) {
        mOnChainListeners.map(onChainListener -> onChainListener.onEventStart(event));
    }
    ...
    public void onEventError(Event event, Throwable throwable) {
        mOnChainListeners.map(onChainListener -> onChainListener.onEventError(event, throwable));
    }

    @Override
    public void onEventNext(Event event, Object result) {
        mOnChainListeners.map(onChainListener -> onChainListener.onEventNext(event, result));
